...
class UsersController extends Controller {
    ...
    public function profile(Request $request, User $user = null) {
        ...
        // Only allow viewing if it's the user's own profile or if they're an admin
    ...
    public function edit(Request $request, User $user = null) {
        ...
        // Only allow editing if it's the user's own profile or if they're an admin
        if (auth()->id() != $user->id && !auth()->user()->hasRole('admin')) {
            abort(403, 'Unauthorized action.');
        }
    ...
    public function save(Request $request, User $user) {
        // Only allow saving if it's the user's own profile or if they're an admin
        if (auth()->id() != $user->id && !auth()->user()->hasRole('admin')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        // Only allow role and permission management for admins
        if (auth()->user()->hasRole('admin')) {
            if ($request->roles) {
